...
from mkpy.plugins import wayv
from mkpy.plugins import modulex
from mkpy.plugins import mk_channel
from mkpy.plugins import mk_delay
from mkpy.plugins import mk_eq
from mkpy.plugins import simple_fader
from mkpy.plugins import simple_reverb
from mkpy.plugins import sidechain_comp
from mkpy.plugins import trigger_fx
from mkpy.plugins import xfade
from mkpy.plugins import mk_compressor
from mkpy.plugins import mk_vocoder
from mkpy.plugins import mk_limiter
...
PLUGIN_UI_TYPES = {
    1: euphoria.euphoria_plugin_ui,
    2: rayv2.rayv_plugin_ui,
    3: wayv.wayv_plugin_ui,
    4: modulex.modulex_plugin_ui,
    5: mk_delay.mkdelay_plugin_ui,
    6: mk_eq.mkeq_plugin_ui,
    ...
    11: mk_channel.mkchnl_plugin_ui,
    12: xfade.xfade_plugin_ui,
    13: mk_compressor.mk_comp_plugin_ui,
    14: mk_vocoder.mk_vocoder_plugin_ui,
    15: mk_limiter.mk_lim_plugin_ui,
}

PORTMAP_DICT = {
    "Euphoria": euphoria.EUPHORIA_PORT_MAP,
    "Way-V": wayv.WAYV_PORT_MAP,
    "Ray-V": rayv2.RAYV_PORT_MAP,
    "Modulex": modulex.MODULEX_PORT_MAP,
    "MK Channel": mk_channel.MKCHNL_PORT_MAP,
    "MK Compressor": mk_compressor.MK_COMP_PORT_MAP,
    "MK Delay": mk_delay.MKDELAY_PORT_MAP,
    "MK EQ": mk_eq.MKEQ_PORT_MAP,
    ...
    "MK Vocoder": mk_vocoder.MK_VOCODER_PORT_MAP,
    "MK Limiter": mk_limiter.MK_LIM_PORT_MAP,
}
...
class MkPluginUiDict:
    def __init__(self, a_project, a_ipc, a_stylesheet):
        """ a_project:    glbl.AbstractProject
            a_ipc:        glbl.AbstractIPC
...
class AbstractPluginSettings:
    ...
    def cut(self):
        ...
        self.copy()
        glbl.PLUGIN_UI_DICT.hide_plugin_ui(self.plugin_uid)
    ...
    def paste(self):
        ...
        if PLUGIN_SETTINGS_IS_CUT:
            ...
            self.plugin_uid = glbl.PROJECT.get_next_plugin_uid()
            glbl.PROJECT.copy_plugin(
    ...
    def on_plugin_change(self, a_val=None, a_save=True):
        ...
        if f_index == 0:
            if self.plugin_ui:
                ...
                self.plugin_ui = None
            glbl.PLUGIN_UI_DICT.close_plugin_ui(self.plugin_uid)
            self.plugin_uid = -1
    ...
    def on_show_ui(self):
        ...
        if self.plugin_ui:
            ...
            self.vlayout.removeWidget(self.plugin_ui.widget)
        self.plugin_ui = glbl.PLUGIN_UI_DICT.open_plugin_ui(
...
class PluginRackTab:
    ...
    def set_tooltips(self, a_enabled):
        self.widget.setToolTip(
            mk_strings.PluginRack if a_enabled else "")
...
class MixerChannel:
    ...
    def set_plugin(self, a_graph_dict, a_plugin_dict):
        """ Update the routes and plugins
            ...
            @a_graph_dict:  A glbl.mk_project.RoutingGraph.graph[
                                track_number] node:
                            {send_index: glbl.mk_project.TrackSend, ...}
    ...
    def set_project(self, a_project):
        """ Load the project
            ...
            @a_project: A instance of a class inheriting glbl.AbstractProject
        """
...
class MixerWidget:
    ...
    def update_sends(self, a_graph, a_plugins):
        """ Update the mixer, show channels for active routings
            ...
            @a_graph:   A glbl.mk_project.RoutingGraph
    ...
    def set_tooltips(self, a_enabled):
        self.widget.setToolTip(mk_strings.Mixer if a_enabled else "")
