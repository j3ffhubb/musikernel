...
from sgui.glbl import mk_project
...
class AudioSeqItem(widgets.QGraphicsRectItemNDL):
    ...
    def __init__(
        ...
        for f_painter_path in self.painter_paths:
            ...
            f_y_pos += self.y_inc
        f_file_name = glbl.PROJECT.get_wav_name_by_uid(
            self.audio_item.uid)
        f_file_name = glbl.PROJECT.timestretch_lookup_orig_path(
            f_file_name)
    ...
    def set_brush(self, a_index=None):
        if self.isSelected():
            ...
            if a_index is None:
                self.setBrush(glbl.DEFAULT_TRACK_COLORS[
                self.audio_item.lane_num % len(glbl.DEFAULT_TRACK_COLORS)])
            else:
                self.setBrush(glbl.DEFAULT_TRACK_COLORS[
                    a_index % len(glbl.DEFAULT_TRACK_COLORS)])
    ...
    def get_file_path(self):
        return glbl.PROJECT.get_wav_path_by_uid(self.audio_item.uid)
    ...
    def mouseMoveEvent(self, a_event):
        if glbl.IS_PLAYING or self.event_pos_orig is None:
            return
        if self.is_amp_curving or self.is_amp_dragging:
            f_pos = a_event.pos()
            f_y = f_pos.y()
            ...
            if f_was_stretching:
                ...
                for f_stretch_item in f_stretched_items:
                    ...
                    if (
                        ...
                        print("Error creating " + dest_path)
                    else:
