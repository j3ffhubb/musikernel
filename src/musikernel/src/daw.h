...
void g_dn_song_get(t_daw*, int);
t_dn_routing_graph * g_dn_routing_graph_get(t_daw *);
void v_dn_routing_graph_free(t_dn_routing_graph*);
t_dn_region * g_dn_region_get(t_daw*);
t_dn_atm_region * g_dn_atm_region_get(t_daw*);
void v_dn_atm_region_free(t_dn_atm_region*);
void g_dn_item_get(t_daw*, int);

t_daw * g_daw_get();
int i_dn_get_region_index_from_name(t_daw *, int);
void v_dn_set_is_soloed(t_daw * self);
void v_dn_set_loop_mode(t_daw * self, int a_mode);
void v_dn_set_playback_cursor(t_daw*, double);
int i_dn_song_index_from_region_uid(t_daw*, int);
void v_dn_update_track_send(t_daw * self, int a_lock);
void v_dn_process_external_midi(t_daw * data,
        t_pytrack * a_track, int sample_count, int a_thread_num,
        t_dn_thread_storage * a_ts);
void v_dn_offline_render(t_daw*, double, double, char*, int, int);
void v_dn_audio_items_run(t_daw*, t_dn_item_ref*,
    int, MKFLT**, MKFLT**, int*, t_dn_thread_storage*);

void v_dn_paif_set_control(t_daw*, int, int, int, MKFLT);

void v_dn_song_free(t_dn_song *);
void v_dn_process_note_offs(t_daw * self, int f_i, t_dn_thread_storage*);
void v_dn_process_midi(
    t_daw *, t_dn_item_ref*, int, int, int, long, t_dn_thread_storage*);
void v_dn_zero_all_buffers(t_daw * self);
void v_dn_panic(t_daw * self);

void v_dn_process_atm(t_daw * self, int f_track_num,
...
/* void v_dn_zero_all_buffers(t_data * self)
 */
void v_dn_zero_all_buffers(t_daw * self){
    int f_i;
    MKFLT ** f_buff;
...
void v_dn_osc_send(t_osc_send_data * a_buffers){
    ...
    if(musikernel->osc_queue_index > 0){
        ...
        {
            v_ui_send("musikernel/daw", a_buffers->f_tmp1);
        }
    }
}
...
void v_dn_process_track(t_daw * self, int a_global_track_num,
...
inline void v_dn_process(t_thread_args * f_args)
{
    t_pytrack * f_track;
    int f_track_index;
    t_daw * self = daw;
    ...
    t_dn_thread_storage * f_ts = &daw->ts[f_args->thread_num];

    if(f_args->thread_num > 0)
    {
        memcpy(f_ts, &daw->ts[0], sizeof(t_dn_thread_storage));
    }
...
inline void v_dn_process_atm(
    t_daw * self, int f_track_num, int f_index, int sample_count,
...
void v_dn_process_midi(t_daw * self, t_dn_item_ref * a_item_ref,
...
void v_dn_process_note_offs(t_daw * self, int f_i,
        t_dn_thread_storage * a_ts)
{
...
void v_dn_process_external_midi(t_daw * self,
        t_pytrack * a_track, int sample_count, int a_thread_num,
        t_dn_thread_storage * a_ts)
{
...
inline void v_dn_set_time_params(t_daw * self, int sample_count)
{
        ...
        MKFLT **a_output, MKFLT *a_input_buffers)
{
    ...
    for(f_period = 0; f_period < self->seq_event_result.count; ++f_period)
    {
        ...
        v_dn_process((t_thread_args*)musikernel->main_thread_args);
...
void v_dn_audio_items_run(
    ...
    t_audio_items * f_region = f_item->audio_items;
    ...
    while(f_index_pos < f_region->index_counts[0])
    {
        ...
        t_audio_item * f_audio_item = f_region->items[f_i];
...
void g_dn_song_get(t_daw* self, int a_lock)
{
    t_dn_song * f_result;
...
void v_dn_open_project(int a_first_load)
{
    ...
    sprintf(daw->region_folder, "%s%sregions%s",
        daw->project_folder, PATH_SEP, PATH_SEP);
    ...
    while(f_i < DN_MAX_ITEM_COUNT)
    {
        ...
        {
            g_dn_item_free(daw->item_pool[f_i]);
    ...
    v_dn_update_track_send(daw, 0);

    v_dn_set_is_soloed(daw);
    ...
    //v_dn_set_playback_cursor(daw, 0.0f);
}

t_dn_atm_region * g_dn_atm_region_get(t_daw * self)
{
    int f_i2;
...
t_dn_region * g_dn_region_get(t_daw* self)
{
    ...
    while(1)
    {
        ...
        else  //item reference
        {
            ...
            {
                g_dn_item_get(daw, f_item_ref->item_uid);
            }
...
void g_dn_item_get(t_daw* self, int a_uid)
{
    MKFLT f_sr = musikernel->thread_storage[0].sample_rate;
...
/* void v_dn_set_playback_mode(t_data * self,
 * int a_mode, //
...
void v_dn_set_playback_cursor(t_daw * self, double a_beat)
{
...
void v_dn_offline_render_prep(t_daw * self)
{
...
void v_dn_offline_render(
    ...
    MKFLT f_elapsed = (MKFLT)v_print_benchmark(
...
t_dn_routing_graph * g_dn_routing_graph_get(t_daw * self)
{
