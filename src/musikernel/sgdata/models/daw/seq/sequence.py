...
class Sequence:
    ...
    def compile(
        ...
        """
            @sr:           int, The sample rate, in hertz
            @items:        ProjectFolder, The items for this project
            @audio_p:      ProjectFolder, The audio patterns for this project
            @midi_p:       ProjectFolder, The midi patterns for this project
            @wav_pool:     WavPool, the wave pool for this project
            @out_dir:      AbstractProjectNestedFolder, The directory
                           to output events
            active_tracks: set-of-int, The active track numbers
        """
        self.item_refs.sort(
            key=lambda x: x.pos
        )
        tempo_map = TempoMap.factory(self.tempo_markers, sr)
